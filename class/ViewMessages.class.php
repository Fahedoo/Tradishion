...
class ViewMessages extends ViewPrivate {
    ...
    public function __construct($userData) {
        ...
        $this->pageTitle = $this->t('menu_messages') . " - TradiShion";
    ...
    protected function getBodyContent() {
        ...
        ?>
        <div class="chat-container" style="display:flex; max-width:1500px; margin:10px auto 18px; height:calc(100vh - 132px); gap:18px; padding:0 8px;">
            
            <div class="chat-sidebar-wrap" style="display:flex; flex-direction:column; gap:14px; width:380px; height:100%; flex-shrink:0;">
                
                <div class="card nav-menu" style="display:none; margin-bottom:0; flex-shrink:0;">
                   <ul style="list-style: none; padding: 0; margin: 0;">
                        <li><a href="index.php?page=dashboard">🏠 <?php echo $this->t('menu_home'); ?></a></li>
                        <li><a href="index.php?page=explorer">🧭 <?php echo $this->t('menu_explorer'); ?></a></li>
                        <li><a href="index.php?page=events">📅 <?php echo $this->t('menu_events'); ?></a></li>
                        <li><a href="index.php?page=messages" class="active">💬 <?php echo $this->t('menu_messages'); ?></a></li>
                        <li><a href="index.php?page=network">👥 <?php echo $this->t('menu_network'); ?></a></li>
                    </ul>
                </div>

                <aside class="chat-sidebar card" style="flex:1; display:flex; flex-direction:column; padding:0; overflow:hidden; margin-bottom:0; border-radius:18px;">
                    <div style="padding:20px; border-bottom:1px solid #eee;">
                        <h3 style="margin-bottom:15px; margin-top:0; font-family:'Libre Baskerville', serif;"><?php echo $this->t('msg_discussions'); ?></h3>
                        <input type="text" placeholder="<?php echo $this->t('msg_search'); ?>" style="width:100%; padding:10px; border-radius:8px; border:1px solid #ddd; background:#f9f9f9; outline:none;">
                    </div>
                    
                    <div class="contact-list" style="overflow-y:auto; flex:1;">
                        <?php if (empty($myConnections)): ?>
                            <p style="padding:20px; text-align:center; color:#888; font-style:italic;"><?php echo $this->t('msg_no_contacts'); ?></p>
                        <?php else: ?>
                            <?php foreach ($myConnections as $contact): ?>
                                <?php $initial = $this->getTextInitial($contact['display_name']); ?>
                                <div class="contact-item" data-id="<?php echo $contact['id_user']; ?>" data-name="<?php echo htmlspecialchars($contact['display_name']); ?>" style="display:flex; align-items:center; gap:15px; padding:15px 20px; border-bottom:1px solid #eee; cursor:pointer;">
                                    ...
                                    <div style="flex:1;">
                                        ...
                                        <p style="margin:0; font-size:0.85rem; color:#666; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">🌍 <?php echo htmlspecialchars($this->formatOrigins($contact['origins'] ?? [])); ?></p>
                                    </div>
                                </div>
                            <?php endforeach; ?>
                        <?php endif; ?>
            ...
            <main class="chat-window card" style="flex:1; display:flex; flex-direction:column; padding:0; overflow:hidden; border-radius:18px; min-width:0;">
                <div style="padding:15px 20px; border-bottom:1px solid #eee; display:flex; justify-content:space-between; align-items:center; background: #fafafa;">
                    <div style="display:flex; align-items:center; gap:15px;">
                        <button type="button" class="chat-back-btn" id="chat-back-btn" style="background:none; border:none; font-size:1.3rem; cursor:pointer; color:#A64B35; padding:4px;">←</button>
                        <div>
                            <h4 id="chat-header-name" style="margin:0;"><?php echo $this->t('msg_select_chat'); ?></h4>
                            <small style="color:#4caf50;"><?php echo $this->t('msg_online'); ?></small>
                        </div>
                    </div>
                ...
                <div id="chat-history" style="flex:1; padding:20px; overflow-y:auto; background:#fcfcfc; display:flex; flex-direction:column; gap:15px;">
                    <p style="text-align:center; color:#888; margin-top: 50px;"><?php echo $this->t('msg_empty_history'); ?></p>
                </div>
                ...
                <div style="padding:15px 20px; border-top:1px solid #eee; display:flex; align-items:center; gap:15px; background:white; position:relative;">
                    ...
                    <input type="text" id="chat-input" placeholder="<?php echo $this->t('msg_placeholder'); ?>" style="flex:1; padding:12px 20px; border-radius:25px; border:1px solid #eee; background:#f4f5f7; outline:none; font-size:0.95rem;">
        ...
        <script>
            ...
            function loadChat(contactId, contactName) {
                ...
                chatHistory.innerHTML = `<p style="text-align:center; color:#888; padding:20px;">${window.Lang.loading}</p>`;
                ...
                fetch(`index.php?page=api_get_chat&contact=${contactId}`)
                    ...
                    .then(messages => {
                        ...
                        if(messages.length === 0) { chatHistory.innerHTML = `<p style="text-align:center; color:#888; margin-top:20px;">${window.Lang.msg_no_messages}</p>`; return; }
            ...
            function sendMessage() {
                ...
                .then(res => res.json()).then(data => {
                    ...
                    } else { showToast(window.Lang.msg_spam_error, false); }
                });
            }
            ...
            if (window.innerWidth <= 768) {
                // On mobile, don't auto-open first chat
                if (chatContainer) chatContainer.classList.remove('mobile-chat-open');
            }
